...
Процедура СобратьСтатистикуКонфигурации() Экспорт
	...
	МодульЦентрМониторинга.ЗаписатьСтатистикуОбъектаКонфигурации("ДатаЗапретаУстановлена", Выборка.Количество > 0);
...
Функция ЗначениеПоля(ЗначенияПолей, Поле)
	...
	Ссылка = ЗначенияПолей[Поле.Имя];
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕстьNULL(" + Поле.Путь + ", Неопределено) КАК ЗначениеРеквизита
	|ИЗ
...
Функция ПолеТаблицы(Источник, ВидПоля, ОбъектМетаданных, ЭтоРегистр)
	...
	Свойства = Новый Структура("Имя, ТабличнаяЧасть, Путь, ИмяИПуть");
	...
	ПозицияТочки = Найти(Свойства.ИмяИПуть, ".");
	Если ПозицияТочки > 0 Тогда
