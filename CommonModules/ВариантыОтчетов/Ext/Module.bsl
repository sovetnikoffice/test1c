...
Функция ДеревоНастроекВариантовОтчетовКонфигурации() Экспорт
	Возврат ДеревоПредопределенных("Внутренний");
КонецФункции
...
//
// Параметры:
//   ФильтрПоТипуОтчетов - Неопределено, Строка - Необязательный. Типы отчетов, для которых требуется сформировать дерево настроек.
//       * Неопределено - Сформировать информацию по отчетам конфигурации и подключенных расширений.
//       * "Внутренний" - Значение по умолчанию. Сформировать информацию по отчетам конфигурации.
//       * "Расширение" - Сформировать информацию по отчетам подключенных расширений.
//
// Возвращаемое значение:
//   ДеревоВариантов - ДеревоЗначений - Настройки предопределенных вариантов отчетов, подключенных к подсистеме.
//     
...
Функция ДеревоПредопределенных(ФильтрПоТипуОтчетов = "Внутренний") Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		СтрокаОтчет.ЭтоВариант           = Ложь;
		СтрокаОтчет.ФорматНастроекСКД    = ПостроенНаСКД И Не ЕстьРеквизиты;
	...
	Для Каждого СтрокаОтчет Из ДеревоВариантов.Строки Цикл
		...
		Если СтрокаОтчет.ФорматНастроекСКД Тогда
			ПодключенКОсновнойФорме = ОтчетПодключенКОсновнойФорме(СтрокаОтчет.Метаданные, КэшФлажкаОсновнойФормы);
			ПодключенКФормеНастроек = ОтчетПодключенКФормеНастроек(СтрокаОтчет.Метаданные, КэшФлажкаФормыНастроек);
			Если ПодключенКОсновнойФорме <> ПодключенКФормеНастроек Тогда
				ТекстОшибки = ВариантыОтчетовКлиентСервер.ТекстОшибки(?(ПодключенКОсновнойФорме, 1, 2), СтрокаОтчет.Метаданные.ПолноеИмя());
				Если ОбщегоНазначенияКлиентСервер.РежимОтладки() Тогда
					ОшибкаПоВарианту(ОтчетСсылка, ТекстОшибки);
				Иначе
					ПредупреждениеПоВарианту(ОтчетСсылка, ТекстОшибки);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
