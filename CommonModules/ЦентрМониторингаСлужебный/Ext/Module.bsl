...
Функция ПолучитьРегламентноеЗадание(ИмяРегламентногоЗадания, СоздатьНовое = Истина) Экспорт
...
Процедура УстановитьРасписаниеПоУмолчанию(Задание) Экспорт
	Задание.Расписание.ПериодПовтораДней = 1;
	Задание.Расписание.ПериодПовтораВТечениеДня = 600;
	Задание.Записать();
КонецПроцедуры

Процедура УдалитьРегламентноеЗадание(ИмяРегламентногоЗадания) Экспорт
...
Процедура РазобратьБуферОперацийСтатистики(ТекущаяДата) Экспорт
	ОценкаПроизводительностиСуществует = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности");
	Если ОценкаПроизводительностиСуществует Тогда
		МодульОценкаПроизводительностиКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ОценкаПроизводительностиКлиентСервер");
		ВремяНачала = МодульОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени();
...
Процедура АгрегироватьЗамерыОперацийСтатистики(ТекущаяДата) Экспорт
	ОценкаПроизводительностиСуществует = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности");
	Если ОценкаПроизводительностиСуществует Тогда
		МодульОценкаПроизводительностиКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ОценкаПроизводительностиКлиентСервер");
		ВремяНачала = МодульОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени();
...
Процедура РегистрацияОперацийСтатистики() Экспорт
	ОценкаПроизводительностиСуществует = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности");
	Если ОценкаПроизводительностиСуществует Тогда
		МодульОценкаПроизводительностиКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ОценкаПроизводительностиКлиентСервер");
		ВремяНачала = МодульОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени();
...
Функция СформироватьJSON(ИмяСекции, Данные, ДатаНачала = Неопределено, ДатаОкончания = Неопределено, ДопПараметры = Неопределено, КолонкиИндекса = Неопределено) Экспорт
	Если ТипЗнч(Данные) = Тип("РезультатЗапроса") Тогда
		...
		ТелоJSON = СтруктураJSONВСтроку(JSONСтруктура); 
	КонецЕсли;
	
	Возврат ТелоJSON;
КонецФункции

Функция СформироватьJSONСтруктуру(ИмяСекции, Данные, ДатаНачала = Неопределено, ДатаОкончания = Неопределено, ДопПараметры = Неопределено, КолонкиИндекса = Неопределено) Экспорт
	Если ТипЗнч(Данные) = Тип("РезультатЗапроса") Тогда
		JSONСтруктура = СформироватьJSONСтруктуруРезультатЗапроса(ИмяСекции, Данные, ДатаНачала, ДатаОкончания, ДопПараметры, КолонкиИндекса)
	КонецЕсли;
...
Функция СформироватьСтруктуруJSONДляОтправки(Параметры) Экспорт
	...
	Если ПараметрыЦентраМониторинга.РегистрироватьДампы Тогда
		...
		СекцияDUMPS = СформироватьJSONСтруктуру("dumps", ТопДампов, ДатаНачала, ДатаОкончания);
	КонецЕсли;
	...
	Если ПараметрыЦентраМониторинга.РегистрироватьБизнесСтатистику Тогда
		...
		СекцияОперацииСтатистики = СформироватьJSONСтруктуру("OperationStatistics", РезультатЗапроса, ДатаНачала, ДатаОкончания,,КолонкиИндекса);
		...
		СекцияКомментарииСтатистики = СформироватьJSONСтруктуру("CommentsStatistics", РезультатЗапроса, ДатаНачала, ДатаОкончания,,КолонкиИндекса);
		...
		СекцияОбластиСтатистики = СформироватьJSONСтруктуру("StatisticalAreas", РезультатЗапроса, ДатаНачала, ДатаОкончания,,КолонкиИндекса);
	...
	Если ПараметрыЦентраМониторинга.РегистрироватьСтатистикуКонфигурации Тогда
		...
		Иначе
			...
			СекцияСтатистикаКонфигурации = СформироватьJSONСтруктуру("StatisticsConfiguration", РезультатЗапроса, ДатаНачала,,ДопПараметры);
		КонецЕсли;
	КонецЕсли;
	#КонецОбласти
	...
	Если ПараметрыЦентраМониторинга.РегистрироватьНастройкуКонфигурации Тогда
		...
		Иначе
			...
			СекцияНастройкаКонфигурации = СформироватьJSONСтруктуру("Options", РезультатЗапроса, ДатаНачала,,ДопПараметры);
		КонецЕсли;
	КонецЕсли;
	#КонецОбласти
	...
	Если ОценкаПроизводительностиСуществует Тогда
		...
		Если ПараметрыЦентраМониторинга.РегистрироватьПроизводительность Тогда
			...
			СекцияТопAPDEX = СформироватьJSONСтруктуру("TopApdex", РезультатЗапроса, ДатаНачала, ДатаОкончания,,КолонкиИндекса);	
		...
		Если ПараметрыЦентраМониторинга.РегистрироватьПроизводительностьТехнологическую Тогда
			...
			КолонкиИндекса.Вставить("Период", Новый Соответствие);
			КолонкиИндекса.Вставить("КлючеваяОперация", Новый Соответствие);
			СекцияТопAPDEXСлужебные = СформироватьJSONСтруктуру("TopApdexTechnology", РезультатЗапроса, ДатаНачала, ДатаОкончания,,КолонкиИндекса);
...
Функция HTTPСервисОтправитьКакТекст(Сервер, АдресРесурса, Данные, Порт = 80, ЗащищенноеСоединение = Ложь) Экспорт
...
Функция HTTPСервисОтправитьКакZIP(Сервер, АдресРесурса, Данные, Порт = 80) Экспорт
	Параметры = Новый Структура;
	Параметры.Вставить("Сервер", Сервер);
	Параметры.Вставить("АдресРесурса", АдресРесурса);
	Параметры.Вставить("Данные", Данные);
	Параметры.Вставить("Метод", "POST");
	Параметры.Вставить("ТипДанных", "ZIP");
...
Функция HTTPОтветВСтруктуру(Ответ) Экспорт
...
Процедура РегистрацияДампов() Экспорт
	ОценкаПроизводительностиСуществует = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности");
	Если ОценкаПроизводительностиСуществует Тогда
		МодульОценкаПроизводительностиКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ОценкаПроизводительностиКлиентСервер");
		ВремяНачала = МодульОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени();
	...
	Для Каждого ФайлДампа Из ФайлыДампов Цикл
		Если ФайлыДамповЗарегистрированные[ФайлДампа.ПолноеИмя] = Неопределено Тогда 
			...
			СтруктураДампа = ПолучитьСтруктуруДампа(ФайлДампа.Имя);
...
Процедура СформироватьПакетДляОтправки() Экспорт
	ОценкаПроизводительностиСуществует = ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности");
	Если ОценкаПроизводительностиСуществует Тогда
		МодульОценкаПроизводительностиКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ОценкаПроизводительностиКлиентСервер");
		ВремяНачала = МодульОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени();
...
Функция ОтправитьДанныеМониторинга() Экспорт
...
Функция УстановитьПараметрыОтправки(Параметры) Экспорт
...
Функция УстановитьПараметрыЦентраМониторинга(НовыеПараметры) Экспорт
...
Функция УдалитьПараметрыЦентраМониторинга() Экспорт
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("Константа.ПараметрыЦентраМониторинга");
	ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
...
Функция УстановитьПараметрЦентраМониторинга(Параметр, Значение) Экспорт
...
Функция НайтиКаталогДампов(КаталогНастроек)
	...
	Если Файл.Существует() Тогда
		...
		Если ПараметрыДампов.type = "0" ИЛИ Найти(КаталогДампов.Путь, "80af5716-b134-4b1c-a38d-4658d1ac4196") > 0 Тогда
...
Функция ПолучитьСтруктуруДампа(Знач ИмяФайла) Экспорт
...
Функция СобытиеЖурналаРегистрацииЦентрМониторингаУдалениеДампа() Экспорт
	Возврат НСтр("ru = 'Центр мониторинга.Удаление дампа'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции

Функция СобытиеЖурналаРегистрацииЦентрМониторингаРазобратьБуферОперацийСтатистики() Экспорт
