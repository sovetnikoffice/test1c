...
//             ** РегионЗагружен - Булево - Имеет смысл только для регионов. Истина, если есть записи.
//
Функция ВариантыАвтоподбораНомеровДомов(ИдентификаторАдресногоОбъекта, Текст) Экспорт
		...
		Пока Запись.Следующий() Цикл
			Если ЗначениеЗаполнено(Запись.Строения) Тогда
				...
				Если Не ПустаяСтрока(Описание) Тогда
					...
					Если Не ПустаяСтрока(Описание) Тогда
						ИзвлечьСписокДомов(Описание, Текст, ВариантыДомов);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	...
	ВариантыДомов.СортироватьПоЗначению();
	Возврат ВариантыДомов;
...
Процедура УстановитьИдентификаторыНаселенногоПункта(ЧастиАдреса, ИдентификаторНаселенногоПункта) Экспорт
	...
	ИдентификаторыАдреса = ИдентификаторыАдреса(ПоляАдреса, Ложь, Истина);
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	Если ЕстьЗаписиДомов И Не ЕстьВарианты Тогда
		...
		Если ЗначениеЗаполнено(ОписаниеЗданияПоСоответствиюФИАС) Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""%1"" отсутствует в адресном классификаторе'"), ОписаниеЗданияПоСоответствиюФИАС(ЗданияИПомещения)));
		КонецЕсли;
...
Процедура ЗаполнитьКодыАдреса(Адрес) Экспорт
	...
	ТекстСоединения = "";
	ТекстДляВыбрать = "";
	ИмяОбъектаДляДополнительныхСведений = "АдресныйОбъектРегион";
	...
	Иначе
		...
		|	И АдресныеОбъектыРайон.КодОкруга = " + сОкруг + "
		|	И АдресныйОбъектРегион.КодСубъектаРФ = АдресныеОбъектыРайон.КодСубъектаРФ
		|	И АдресныеОбъектыРайон.Уровень = 3
		|	И АдресныеОбъектыРайон.КодГорода = 0
		|	И АдресныеОбъектыРайон.КодВнутригородскогоРайона = 0
		|	И АдресныеОбъектыРайон.КодНаселенногоПункта = 0
	...
	Иначе
		...
		|	И АдресныйОбъектРегион.КодСубъектаРФ = АдресныеОбъектыГород.КодСубъектаРФ
		|	И АдресныеОбъектыГород.Уровень = 4
		|	И АдресныеОбъектыГород.КодОкруга = " + сОкруг + "
		|	И АдресныеОбъектыГород.КодРайона = " + сРайон + "
		|	И АдресныеОбъектыГород.КодВнутригородскогоРайона = 0
	...
	Иначе
		...
		|	И АдресныйОбъектРегион.КодСубъектаРФ = АдресныеОбъектыВнутригородскойРайон.КодСубъектаРФ
		|	И АдресныеОбъектыВнутригородскойРайон.КодОкруга = "  + сОкруг + "
		|	И АдресныеОбъектыВнутригородскойРайон.КодРайона = " + сРайон + "
		|	И АдресныеОбъектыВнутригородскойРайон.КодГорода = " + сГород + "
		|	И АдресныеОбъектыВнутригородскойРайон.КодНаселенногоПункта = 0
	...
	Иначе
		...
		|	И АдресныйОбъектРегион.КодСубъектаРФ = АдресныеОбъектыНаселенныйПункт.КодСубъектаРФ
		|	И АдресныеОбъектыНаселенныйПункт.Уровень = 6
		|	И АдресныеОбъектыНаселенныйПункт.КодОкруга = "  + сОкруг + "
		|	И АдресныеОбъектыНаселенныйПункт.КодРайона = " + сРайон + "
		|	И АдресныеОбъектыНаселенныйПункт.КодГорода = " + сГород + "
	...
	Иначе
		...
		|	И АдресныйОбъектРегион.КодСубъектаРФ = АдресныеОбъектыУлица.КодСубъектаРФ
		|	И АдресныеОбъектыУлица.Уровень = 7
		|	И АдресныеОбъектыУлица.КодОкруга = "  + сОкруг + "
		|	И АдресныеОбъектыУлица.КодРайона = " + сРайон + "
		|	И АдресныеОбъектыУлица.КодГорода = " + сГород + "
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	АдресныйОбъектРегион.КодКЛАДР КАК КодКЛАДРРегион,
	|	АдресныйОбъектРегион.КодСубъектаРФ КАК КодСубъектаРФ, " + ТекстДляВыбрать + "
...
Процедура ПолныеНаименованияСокращений(СоответствиеСокращений) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			И (ТаблицаСокращений.Сокращение ПОДОБНО УровниСокращенийАдресныхСведений.Сокращение)";
	...
	ТаблицаСоответствиеСокращений.Колонки.Добавить("Сокращение", Новый ОписаниеТипов("Строка"));
...
//
Процедура ИзвлечьСписокДомов(СтрокаСоСпискомДомов, СтрокаПоиска, ВариантыДомов)
	...
	Пока Позиция <= ЧислоСтрок Цикл
		...
		Если Строка = "H" Тогда
			ВидВладения   = ТипЧисло.ПривестиЗначение(СтрПолучитьСтроку(СтрокаСоСпискомДомов, Позиция + 1));
			НомерВладения = СокрЛП( СтрПолучитьСтроку(СтрокаСоСпискомДомов, Позиция + 2) );
			НомерКорпуса  = СокрЛП( СтрПолучитьСтроку(СтрокаСоСпискомДомов, Позиция + 3) );
			ВидСтроения   = ТипЧисло.ПривестиЗначение(СтрПолучитьСтроку(СтрокаСоСпискомДомов, Позиция + 4));
			НомерСтроения = СокрЛП( СтрПолучитьСтроку(СтрокаСоСпискомДомов, Позиция + 5) );
			
			Если ЗначениеЗаполнено(НомерВладения) Тогда
				Если СтрНачинаетсяС(НомерВладения, СтрокаПоиска) 
					И ВариантыДомов.НайтиПоЗначению(НомерВладения) = Неопределено Тогда
						ВариантыДомов.Добавить(НомерВладения, НомерВладения);
				КонецЕсли;
			КонецЕсли;
			Позиция = Позиция + 5;
		КонецЕсли;
