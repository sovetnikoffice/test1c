...
Функция СформироватьПрограммноНаСервере(СхемаКомпоновкиДанных, Настройки, УникальныйИдентификатор, АдресХранилища) Экспорт
	...
	ПКД.Инициализировать(МакетКомпоновки,,ДанныеРасшифровки);
	...
	Если Результат.Рисунки.Количество() = 1 Тогда
		...
		Результат.Рисунки[0].Объект.Анимация = АнимацияДиаграммы.НеИспользовать;
		Результат.Рисунки[0].Объект.ОбластьПостроения.Лево = 0;
		Результат.Рисунки[0].Высота = 60;
		Результат.Рисунки[0].Ширина = 250;
		...
		КонецЦикла;
		Результат.УдалитьОбласть(Результат.Область("R20:R35"), ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
