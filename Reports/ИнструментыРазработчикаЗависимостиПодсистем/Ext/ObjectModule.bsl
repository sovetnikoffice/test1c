...
Процедура ПолучитьДокументированныеСвязиПодсистем()
	...
	Пока ЧтениеХМЛ.Прочитать() Цикл
		...
		Если ЧтениеХМЛ.ЛокальноеИмя = "Подсистема"
			...
			ЗависитОтПодсистем = СтрРазделить(ЧтениеХМЛ.ПолучитьАтрибут("ЗависимыеПодсистемы"), ",", Ложь);
			УсловноЗависитОтПодсистем = СтрРазделить(ЧтениеХМЛ.ПолучитьАтрибут("УсловноЗависитОт"), ",", Ложь);
...
Процедура ВыполнитьПоискВызововВДругиеПодсистемы(ВызывающаяПодсистема, ВызываемаяПодсистема, ПолноеИмяВызывающегоОбъекта, ИсходнаяСтрока, ЭлементСоответствия)
	...
	Если СтрНайти(СтрокаОбращенияИзКода, "БиблиотекаКартинок") > 0
		...
		Или СтрНайти(СтрокаОбращенияИзКода, "ШрифтыСтиля") > 0 Тогда
