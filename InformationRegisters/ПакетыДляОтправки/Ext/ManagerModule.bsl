...
Процедура УдалитьПакеты(НомерПакета) Экспорт
	Запрос = Новый Запрос;
	...
	Запрос.УстановитьПараметр("КрайнийПакет", НомерПакета);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
...
Функция ПолучитьДатуКрайнегоПакета() Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	ISNULL(МАКСИМУМ(ПакетыДляОтправки.Период), ДАТАВРЕМЯ(1,1,1)) КАК ДатаКрайнегоПакета
	|ИЗ
	|	РегистрСведений.ПакетыДляОтправки КАК ПакетыДляОтправки
	|";
	Результат = Запрос.Выполнить();
	...
	Возврат Выборка.ДатаКрайнегоПакета;
КонецФункции
