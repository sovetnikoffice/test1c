...
Процедура ОпределитьНастройки(Настройки, ИдентификаторНастройки) Экспорт
	...
	Настройки.ЗаголовокКомандыДляСозданияНовогоОбменаДанными = НСтр("ru = '1С:Библиотека стандартных подсистем, редакция 2.2 (без использования правил обмена)'");
...
Функция ИмяФайлаНастроекДляПриемника() Экспорт
	...
	Возврат НСтр("ru = 'Настройки синхронизации для БСП (без использования правил)'");
...
Функция КраткаяИнформацияПоОбмену(ИдентификаторНастройки) Экспорт
	...
	ПоясняющийТекст = НСтр("ru = 'Позволяет синхронизировать данные между двумя программами 1С:Библиотека стандартных подсистем, редакция 2.2. 
...
Функция ПодробнаяИнформацияПоОбмену(ИдентификаторНастройки) Экспорт
	...
	Возврат "http://its.1c.ru/db/bspdoc#content:120:1:IssOgl2_Обмен%2520с%2520БСП%2520(без%2520использования%2520правил%2520обмена)";
