...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементыНижнейСтроки = Новый Массив();
	ВыбратьВсеПодчиненныеЭлементы(Элементы.НижняяСтрока, ЭлементыНижнейСтроки);
	Для каждого ЭлементФормы Из ЭлементыНижнейСтроки Цикл
	...
	Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных()
		И ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
			Элемент = УсловноеОформление.Элементы.Добавить();
			ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
			ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.Автор.Имя);
