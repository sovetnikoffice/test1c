...
Процедура ЗаполнитьПараметрыОтправки(ПараметрыОтправки, МассивОбъектов, КоллекцияПечатныхФорм) Экспорт
	...
	Если СведенияОПокупателе <> Неопределено И ЗначениеЗаполнено(СведенияОПокупателе.Партнер) Тогда
		...
		АдресаПартнера = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СведенияОПокупателе.Партнер), 
		...
		Для Каждого Адрес Из АдресаПартнера Цикл
			...
			Если Не ПустаяСтрока(ПредставлениеАдреса) Тогда
				...
				ОписаниеПолучателя.Вставить("Адрес", ПредставлениеАдреса);
				ОписаниеПолучателя.Вставить("Представление", ПредставлениеПолучателя);
				ОписаниеПолучателя.Вставить("ИсточникКонтактнойИнформации", СведенияОПокупателе.Партнер);
				ОписаниеПолучателя.Вставить("ВидПочтовогоАдреса", Строка(Адрес.Вид));
				ОписаниеПолучателя.Вставить("Пояснение", Пояснение);
