...
//   * ПомещенныеФайлы - Массив - со значениями типа ОписаниеПереданногоФайла
//                     - Неопределено - если файлы с описаниями ролей внешних пользователей не получены.
//   * ОшибкиВФайлах   - Строка - (возвращаемое значение) - не пустое, если в файлах, указанных
//                       в параметре ПомещенныеФайлы найдены ошибки.
...
Процедура ПроверитьИспользованиеОграниченийДоступа(ВсеПараметры) Экспорт
	...
	ВсеПараметры.Вставить("ОшибкиВФайлах",   "");
	ВсеПараметры.Вставить("НайденныеОшибки", Неопределено);
	
	НазначениеРолей = РолиВнешнихПользователей(ВсеПараметры.ПомещенныеФайлы, ВсеПараметры.ОшибкиВФайлах);
	...
	ПараметрыПроцедуры.Вставить("НазначениеРолей",     НазначениеРолей);
...
Процедура ОпределитьВидыОграниченийПрав(Параметры) Экспорт
	...
	РолиТолькоДляВнешнихПользователей         = НазначениеРолей.ТолькоДляВнешнихПользователей;
	РолиДляПользователейИВнешнихПользователей = НазначениеРолей.ДляПользователейИВнешнихПользователей;
	...
	Для Каждого Строка Из ОграниченияДоступа Цикл
		...
		Если СтрНачинаетсяС(Свойства.Роль, "Профиль") Тогда
			Продолжить;
		КонецЕсли;
		...
		НоваяСтрока.РольДляВнешнихПользователей =
			РолиТолькоДляВнешнихПользователей.Найти(Свойства.Роль) <> Неопределено
			Или РолиДляПользователейИВнешнихПользователей.Найти(Свойства.Роль) <> Неопределено;
...
Процедура ОбновитьПраваПрофильныхРолей(Журнал) Экспорт
	...
	ЗаписатьСообщение(Журнал,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Начало процедуры %1'"),
	...
	ЗаписатьСообщение(Журнал,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Окончание процедуры %1'"),
		Формат(ТекущаяДатаСеанса(), "ДФ='dd.MM.yyyy HH:mm:ss'")));
	
КонецПроцедуры

// Обработчик определения ролей внешних пользователей., которые не будут
// совмещаться с ролями для обычных пользователей у одного пользователя ИБ.
// Тексты ограничений в этих ролях для той же таблицы с тем же правом могут отличаться.
//
// Параметры:
//  ПомещенныеФайлы - Массив - массив помещенных текстовых файлов, которые имеют
//                    РолиТолькоДляВнешнихПользователей и РолиДляПользователейИВнешнихПользователей.
//
//  ОшибкиВФайлах   - Строка - возвращаемое значение, содержит описание ошибок в файлах.
//
// Возвращаемое значение:
//  Структура со свойствами:
//   * РолиТолькоДляВнешнихПользователей         - Массив - имена ролей внешних пользователей,
//       которые не будут совмещаться с ролями для обычных пользователей у одного пользователя ИБ,
//       тексты ограничений в этих ролях для той же таблицы с тем же правом могут отличаться.
//
//   * РолиДляПользователейИВнешнихПользователей - Массив - имена ролей которые могут
//       сочетаться с любыми ролями.
//
Функция РолиВнешнихПользователей(ПомещенныеФайлы, ОшибкиВФайлах) Экспорт
	
	ОписанияТекстов = Новый Массив;
	
	Для Каждого Макет Из ЭтотОбъект.Метаданные().Макеты Цикл
		
		Если Макет.Имя = "_ДемоРолиВнешнихПользователейБСП"
		   И Метаданные.Имя <> "БиблиотекаСтандартныхПодсистемДемо" Тогда
			
			Продолжить;
		КонецЕсли;
		
		Если СтрНайти(Макет.Имя, "РолиВнешнихПользователей") > 0 Тогда
			Описание = Новый Структура;
			Описание.Вставить("Текст", ПолучитьМакет(Макет));
			Описание.Вставить("ЗаголовокОшибки",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Следующие роли, указанные в макете ""%1"" не найдены:'"), Макет.Имя));
			ОписанияТекстов.Добавить(Описание);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
		...
		Описание.Вставить("ЗаголовокОшибки",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Следующие роли, указанные в файле ""%1"" не найдены:'"), ПомещенныйФайл.Имя));
		ОписанияТекстов.Добавить(Описание);
	КонецЦикла;
	
	Роли = Новый Структура;
	Роли.Вставить("ТолькоДляВнешнихПользователей", Новый Массив);
	Роли.Вставить("ДляПользователейИВнешнихПользователей", Новый Массив);
	
	Для Каждого Описание Из ОписанияТекстов Цикл
		НесуществующиеРоли = "";
		
		ДобавитьРолиИзОбласти(Роли.ТолькоДляВнешнихПользователей,
			Описание, "РолиТолькоДляВнешнихПользователей", НесуществующиеРоли);
		
		ДобавитьРолиИзОбласти(Роли.ДляПользователейИВнешнихПользователей,
			Описание, "РолиДляПользователейИВнешнихПользователей", НесуществующиеРоли);
		
		Если ЗначениеЗаполнено(НесуществующиеРоли) Тогда
			ОшибкиВФайлах = ОшибкиВФайлах + Символы.ПС + Символы.ПС
				+ Описание.ЗаголовокОшибки + Символы.ПС + НесуществующиеРоли;
		КонецЕсли;
	КонецЦикла;
	
	ОшибкиВФайлах = СокрЛП(ОшибкиВФайлах);
...
// Для функции РолиВнешнихПользователей.
Процедура ДобавитьРолиИзОбласти(Роли, ОписаниеТекста, ИмяОбласти, НесуществующиеРоли)
	
	Попытка
		Область = ОписаниеТекста.Текст.ПолучитьОбласть(ИмяОбласти);
	Исключение
		НесуществующиеРоли = НесуществующиеРоли + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Область %1 не найдена.'"), ИмяОбласти);
		Возврат;
	КонецПопытки;
	...
	Для НомерСтроки = 1 По СтрЧислоСтрок(ТекстОбласти) Цикл
		Строка = СокрЛП(СтрПолучитьСтроку(ТекстОбласти, НомерСтроки));
		...
		Если СтрНачинаетсяС(Строка, "//") Тогда
			Продолжить;
		КонецЕсли;
		Если Метаданные.Роли.Найти(Строка) = Неопределено Тогда
			НесуществующиеРоли = НесуществующиеРоли + Символы.ПС + Строка;
		ИначеЕсли Роли.Найти(Строка) = Неопределено Тогда
			Роли.Добавить(Строка);
		КонецЕсли;
	КонецЦикла;
