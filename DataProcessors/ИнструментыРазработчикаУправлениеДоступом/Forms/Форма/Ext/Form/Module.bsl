...
&НаКлиенте
Процедура ПапкаСФайламиОписанийРолейВнешнихПользователейНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);
	Диалог.Каталог = ПапкаСФайламиОписанийРолейВнешнихПользователей;
	Диалог.Заголовок = НСтр("ru = 'Выбор папки, содержащей файлы описаний ролей внешних пользователей'");
	
	Диалог.Показать(Новый ОписаниеОповещения(
		"ПапкаСФайламиОписанийРолейВнешнихПользователейПослеВыбора", ЭтотОбъект));
...
&НаКлиенте
Процедура ПапкаСФайламиОписанийРолейВнешнихПользователейПослеВыбора(ВыбранныеФайлы, Контекст) Экспорт
	
	Если ТипЗнч(ВыбранныеФайлы) = Тип("Массив") Тогда
		ПапкаСФайламиОписанийРолейВнешнихПользователей = ВыбранныеФайлы[0];
	КонецЕсли;
...
Процедура СформироватьОписание(ПолноеОписание = Истина)
	...
	Контекст.Вставить("ПолноеОписание", ПолноеОписание);
	
	НачатьПоискФайлов(
		Новый ОписаниеОповещения("СформироватьОписаниеПослеПоискаФайлов", ЭтотОбъект, Контекст),
		ПапкаСФайламиОписанийРолейВнешнихПользователей, "РолиВнешнихПользователей*.txt");
	
КонецПроцедуры

&НаКлиенте
Процедура СформироватьОписаниеПослеПоискаФайлов(НайденныеФайлы, Контекст) Экспорт
	
	Если НайденныеФайлы.Количество() = 0 Тогда
		СформироватьОписаниеПослеПомещенияФайлов(Новый Массив, Контекст);
		Возврат;
	КонецЕсли;
	...
	НачатьПомещениеФайлов(
		Новый ОписаниеОповещения("СформироватьОписаниеПослеПомещенияФайлов", ЭтотОбъект, Контекст),
		ПомещаемыеФайлы, , Ложь, УникальныйИдентификатор);
	
КонецПроцедуры

&НаКлиенте
Процедура СформироватьОписаниеПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	
	НовоеОписание = НовоеОписаниеВидовОграниченийПрав(ПомещенныеФайлы);
	Если НовоеОписание = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Контекст.ПолноеОписание Тогда
		Текст = Новый ТекстовыйДокумент;
...
&НаКлиенте
Функция НовоеОписаниеВидовОграниченийПрав(ПомещенныеФайлы)
	
	ВидыОграниченийПрав.Очистить();
	
	ОшибкиВВыгрузке = "";
	ОшибкиВФайлах = "";
	НайденныеОшибки = ОпределитьВидыОграниченийПравНаСервере(ПомещенныеФайлы, ОшибкиВФайлах, ОшибкиВВыгрузке);
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(ОшибкиВФайлах) Тогда
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.ДобавитьСтроку(ОшибкиВФайлах);
		ТекстовыйДокумент.Показать(НСтр("ru = 'Ошибки в описаниях ролей внешних пользователей'"));
	КонецЕсли;
	Если ЗначениеЗаполнено(ОшибкиВФайлах) Или ЗначениеЗаполнено(ОшибкиВВыгрузке) Тогда
...
&НаСервере
Функция ОпределитьВидыОграниченийПравНаСервере(ПомещенныеФайлы, ОшибкиВФайлах, ОшибкиВВыгрузке)
	
	Обработка = РеквизитФормыВЗначение("Объект");
	НазначениеРолей = Обработка.РолиВнешнихПользователей(ПомещенныеФайлы, ОшибкиВФайлах);
