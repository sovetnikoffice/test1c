...
Функция ИзменитьВыбранныеСервер(РезультатРаботы)
	...
	Для Каждого СтрокаОбъектаМетаданных Из Найденные Цикл
		...
		Если НЕ СтрокаОбъектаМетаданных.ФормаОбъектаПодключена И НЕ СтрокаОбъектаМетаданных.ФормаОбъектаЕстьКритичныеОшибки Тогда
			...
			ПодключитьКодПриСозданииНаСервере   = (Найти(ТекстМодуля, ПоискКода.ПриСозданииНаСервере) = 0);
			ПодключитьКодВыполнитьКоманду       = СтрокаОбъектаМетаданных.ЗаполнениеОбъекта И (Найти(ТекстМодуля, ПоискКода.ВыполнитьКоманду) = 0);
			ПодключитьКодВыполнитьКомандуСервер = СтрокаОбъектаМетаданных.ЗаполнениеОбъекта И (Найти(ТекстМодуля, ПоискКода.ВыполнитьКомандуСервер) = 0);
...
Процедура ЗачитатьДанныеЭтойПрограммы(ВыбранноеЗначение = -1, ПараметрыЧтения = Неопределено) Экспорт
	...
	НетОшибокКоторыеМожноИсправить = НЕ ОпределитьОшибкиВнедренияКоторыеМожноИсправить();
	...
	Если НетОшибокКоторыеМожноИсправить Тогда
