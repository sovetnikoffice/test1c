...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДобавитьПроверку(НСтр("ru = 'Роль и вид доступа должны быть включены хотя бы в одно описание профиля'"), "Подключаемый_ПроверитьВхождениеРолейИВидовДоступаВПрофили()");
	...
	ДобавитьПроверку(НСтр("ru = 'Недопустимо использование метода ""РольДоступна()""'"), "Подключаемый_НайтиВызовыМетодаРольДоступна()", НСтр("ru = 'Требуется закрытие конфигуратора!'"));
	...
	ДобавитьПроверку(НСтр("ru = 'Настройкой доступности элементов формы по ролям необходимо управлять из кода'"), "Подключаемый_НайтиНеподдерживаемыеНастройкиРолей()");
...
Функция Подключаемый_ПроверитьКорректностьКомментированияДемоПримеров()
	...
	Для Каждого ФайлМодуля Из МассивМодулей Цикл
		...
		Если СтрНайти(ФайлМодуля.Имя, "ИнструментыРазработчикаПереходНаВерсию231") > 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		Пока НомерТекущейСтроки < КоличествоСтрок Цикл
			...
			Если СтрНайти(СтрокаПроверки, "Демо ") = 0 Тогда
				...
				Продолжить;
			Иначе
				Если (Найти(СтрокаПроверки, "_Демо начало примера") = 0 И СтрНайти(СтрокаПроверки, "_Демо конец примера") = 0) 
...
Функция Подключаемый_ПроверитьРолиНаНаличиеНедопустимыхПрав()
	...
	РолиИсключения.Добавить("ДобавлениеИзменениеАдресныхСведений");
	РолиИсключения.Добавить("ДобавлениеИзменениеБанков");
	РолиИсключения.Добавить("ДобавлениеИзменениеКалендарныхГрафиков");
	...
	НеразделенныеДанныеДоступныеДляИзменения =
		Константы.ПараметрыРаботыПользователей.СоздатьМенеджерЗначения().НеразделенныеДанныеДоступныеДляИзменения();
	
	Для каждого Роль Из Метаданные.Роли Цикл
		ИмяРоли = Роль.Имя;
		
		Если РолиИсключения.Найти(ИмяРоли) <> Неопределено
...
Функция Подключаемый_ПроверитьВхождениеРолейИВидовДоступаВПрофили()
	...
	РолиИсключения.Вставить("АдминистраторСистемы", Истина);
	...
	Для Каждого Роль Из Метаданные.Роли Цикл
		...
		СписокОшибочныхОбъектов.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Роль ""%1"".'"), ИмяРоли));
	КонецЦикла;
...
Функция Подключаемый_ПроверитьКомментарииДляБлоковКодаПодсистем()
	...
	Для Каждого Файл Из МассивМодулей Цикл
		...
		Если СтрНайти(Файл.Имя, "ИнструментыРазработчикаДополнительныеОтчетыИОбработкиРасстановкаФрагментовКода") > 0
			...
			Или СтрНайти(Файл.Имя, "ИнструментыРазработчикаПереходНаВерсию231") > 0 Тогда
			Продолжить;
		КонецЕсли;
...
Функция ИмяПодсистемыЗаданоНекорректно(УсловныйВызов, СписокИспользуемыхПодсистем)
	...
	Если СтрНайти(УсловныйВызов, "ТехнологияСервиса.") > 0
		Или СтрНайти(УсловныйВызов, "ИнтернетПоддержкаПользователей") > 0 Тогда
		Возврат Ложь;
	КонецЕсли;
