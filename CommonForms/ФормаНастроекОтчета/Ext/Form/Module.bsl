...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИдентификаторТекущегоУзлаКД <> Неопределено Тогда
		ПолныйПутьТекущегоУзлаКД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ПолныйПутьТекущегоУзлаКД");
		...
		КонецЕсли;
		КорневойУзел = ОтчетыКлиентСервер.НайтиЭлементПоПолномуПути(НастройкиКД, ПолныйПутьТекущегоУзлаКД);
...
Процедура ПриЗагрузкеВариантаНаСервере(Настройки)
	...
	Если ТипЗнч(ФормаПараметры.Отбор) = Тип("Структура") Тогда
		ОтчетыСервер.УстановитьФиксированныеОтборы(ФормаПараметры.Отбор, Отчет.КомпоновщикНастроек, НастройкиОтчета);
	КонецЕсли;
